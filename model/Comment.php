...
class Comment_Model extends Model {
	...
	private $_face = array(201, 202, 203, 204, 205, 206, 207, 208, 209, 210);
	...
	public function getAll($where, $limit = '0, 10', $order = 'id', $by = 'DESC') {
		...
		foreach ($res as & $value) {
			//$value['content'] = preg_replace('/\[(\d+)\]/e', "\$this->_parseFace('\\1')", $value['content']);
            $value['content'] = preg_replace_callback('/\[(\d+)\]/', function(){
                return $this->_parseFace('\\1');
            }, $value['content']);
		}
		return $res;	
	}
	...
	private function _parseFace($con) {
		if (in_array($con, $this->_face)) {
			return "<img src=".Wee::$config['web_path']."images/face/face{$con}.gif>";
		} else {
			return '[' . $con .']';
		}	
	}
	...
	public function upRelpy($id) {
		$this->db->table('#@_comment')->where(array('id' => $id))->update("up = up + 1");	
	}
