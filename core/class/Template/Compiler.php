...
class Template_Compiler {
	...
	public function compile($tplCon) {
		...
		*/
        ...
        $pattern = '/(\<\?php(.+?)\?\>)/s';
        $tplCon = preg_replace_callback($pattern, function (){
            return $this->_parseCode('$0');
        }, $tplCon);
	...
	private function _parseCode($code) {
        ...
        $code = preg_replace_callback($pattern, function(){
            return $this->_parseTags('$1');
        } , $code);
		
		$pattern = '/((\$[\w\-\>\x7f-\xff]+)(\.[\w\-\>\.\"\'\[\]\$\x7f-\xff]+)+?)/s';
		$code = preg_replace_callback($pattern, function(){
		    return $this->_parseArray('$0');
        }, $code);
		
		$pattern = '/((\$[\w\-\>\x7f-\xff]+)(\[[\w\-\>\.\"\'\[\]\$\x7f-\xff]+\])+?)/s';
		$code = preg_replace_callback($pattern, function(){
		    return $this->_addQuote('$0');
        }, $code);
		
		$pattern = '/\$([\w\x7f-\xff]+)/s';
		$code = preg_replace_callback($pattern, function(){
		    return $this->_addVar('$1');
        }, $code);
